    ...
    const tools: Tool[] = [
      {
        name: "get_chart_data",
        description:
          "Get chart data and metadata for a specific statistic from XDMoD. Common examples: Jobs realm with total_cpu_hours by person, SUPREMM realm with gpu_time by resource.",
        inputSchema: {
          type: "object",
          properties: {
            realm: {
              type: "string",
              description:
                'The realm: "Jobs" for general job statistics, "SUPREMM" for GPU and performance metrics',
              enum: ["Jobs", "SUPREMM"],
            },
            group_by: {
              type: "string",
              description:
                'How to group the data: "none" (system-wide), "resource" (by compute resource), "person" (by user - Jobs realm only)',
