...
const token = tokenIndex !== -1 && args[tokenIndex + 1] ? args[tokenIndex + 1] : process.env.XDMOD_API_TOKEN;

if (!token) {
  console.log('No API token provided. Use --api-token <token> or set XDMOD_API_TOKEN environment variable');
  process.exit(1);
}
      ...
      if (response.result.content && response.result.content[0]) {
        ...
        if (testName === 'debug_auth_status') {
          ...
          if (data.userRelatedGroupBys?.length > 0) {
            console.log(`  Examples: ${data.userRelatedGroupBys.slice(0, 3).map(g => g.text).join(', ')}`);
          }
