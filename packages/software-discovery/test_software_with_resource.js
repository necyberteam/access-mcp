...
async function testSoftwareWithResource() {
  ...
  try {
    const response1 = await client.get(
      `/api=API_0/${API_KEY}/rp=delta.ncsa.access-ci.org?software=python`
    );
  ...
  try {
    const response2 = await client.get(
      `/api=API_0/${API_KEY}/rp=delta.ncsa.access-ci.org?filter=tensorflow`
    );
  ...
  try {
    const response3 = await client.get(
      `/api=API_0/${API_KEY}/rp=delta.ncsa.access-ci.org?search=gpu`
    );
  ...
  try {
    // Get unfiltered
    const unfiltered = await client.get(
      `/api=API_0/${API_KEY}/rp=delta.ncsa.access-ci.org?include=software_name`
    );
    const unfilteredCount = Array.isArray(unfiltered.data) ? unfiltered.data.length : 0;

    // Get filtered
    const filtered = await client.get(
      `/api=API_0/${API_KEY}/rp=delta.ncsa.access-ci.org?include=software_name&software=python`
    );
    ...
    if (filteredCount < unfilteredCount && filteredCount > 0) {
      console.log(
        `✓ Filter appears to be working! Reduced from ${unfilteredCount} to ${filteredCount} results`
      );
