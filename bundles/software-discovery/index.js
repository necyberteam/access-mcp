...
var BaseAccessServer = class {
  ...
  get httpClient() {
    if (!this._httpClient) {
      ...
      this._httpClient = axios_default.create({
...
var KnowledgeBaseResourceSchema = external_exports.object({
  id: external_exports.string(),
  title: external_exports.string(),
...
// packages/software-discovery/dist/server.js
var SoftwareDiscoveryServer = class extends BaseAccessServer {
  ...
  get sdsClient() {
    if (!this._sdsClient) {
      this._sdsClient = axios_default.create({
  ...
  async searchSoftware(query, resourceFilter) {
  ...
  async listSoftwareByResource(resourceId, limit = 100, searchQuery) {
    const apiKey = process.env.SDS_API_KEY || process.env.VITE_SDS_API_KEY;
    if (!apiKey) {
      return {
        content: [
    ...
    if (searchQuery) {
      ...
      softwareList = softwareList.filter((pkg) => pkg.software_name?.toLowerCase().includes(query) || pkg.software_description?.toLowerCase().includes(query));
    }
  ...
  async getSoftwareDetails(softwareName, resourceId) {
    ...
    const allSoftware = await this.listSoftwareByResource(resourceId, 1e3);
...
// packages/software-discovery/dist/index.js
var server = new SoftwareDiscoveryServer();
