...
async function fetchOrganizations() {
  ...
  try {
    const response = await axios.get(`${API_BASE}/wh2/cider/v1/organizations/`, { timeout: 10000 });
    ...
    const idField = organizations[0].organization_id
      ? "organization_id"
      : organizations[0].info_groupid
        ? "info_groupid"
        : organizations[0].id
          ? "id"
          : organizations[0].groupid
            ? "groupid"
            : null;

    const nameField = organizations[0].organization_name
      ? "organization_name"
      : organizations[0].organization_descriptive_name
        ? "organization_descriptive_name"
        : organizations[0].name
          ? "name"
          : organizations[0].descriptive_name
            ? "descriptive_name"
            : null;
    ...
    const knownResourceOrgIds = [
      178, 467, 471, 476, 561, 563, 653, 844, 848, 856, 1869, 2000, 2058, 2440, 4659, 12964, 14449,
      16235, 19169,
    ];
    ...
    console.log("\n" + "═".repeat(80) + "\n");
    console.log(
      `Coverage: ${mappedOrgs.length}/${knownResourceOrgIds.length} organization IDs mapped`
    );
  } catch (error) {
